// From here https://learn.adafruit.com/multi-tasking-the-arduino-part-3/using-neopatterns 
...
enum  pattern { NONE, RAINBOW_CYCLE, THEATER_CHASE, COLOR_WIPE, SCANNER, FADE, FLICKER };
...
class NeoPatterns : public Adafruit_NeoPixel
{
    ...
    uint16_t Pixel;  // current step within the pattern
        ...
        if((millis() - lastUpdate) > Interval) // time to update
        {
            ...
            switch(ActivePattern)
            {
                ...
                case FADE:
                    FadeUpdate();
                    break;
                case FLICKER:  // Custom animation for candle
                    FlickerUpdate(); 
                    break;
                default:
                    break;
    ...
    void Flicker(uint32_t color1, uint32_t color2, uint16_t steps, uint8_t interval, direction dir = FORWARD)
    {
        ...
        Index = 0;
        Direction = dir;
        //Pixel =0; // random(min,max) --max is EXCLUSIVE
    }
